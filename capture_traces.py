import pyvisa
import numpy as np
import time
import os

# ===== Configuration =====
SCOPE_ADDR = 'USB0::0x2A8D::0x17BC::MY65060111::0::INSTR'
N_TRACES = 2000                 # test with small count first
OUTPUT_DIR = 'traces'
REQUESTED_POINTS = 50000

# ===== LFSR PRNG (must match firmware EXACTLY) =====
LFSR_SEED = 0xACE1BEEF
LFSR_TAPS = 0xD0000001

def lfsr_byte(state):
    out = 0
    for _ in range(8):
        bit = state & 1
        state >>= 1
        if bit:
            state ^= LFSR_TAPS
        out = ((out << 1) | bit) & 0xFF
    return out, state

def generate_plaintexts(n_traces):
    plaintexts = np.zeros((n_traces, 16), dtype=np.uint8)
    state = LFSR_SEED
    for i in range(n_traces):
        for j in range(16):
            pt_byte, state = lfsr_byte(state)
            plaintexts[i, j] = pt_byte
    return plaintexts

def configure_scope(scope):
    scope.write(':TIMebase:SCALe 0.0005')
    scope.write(':TIMebase:POSition 0.002')
    
    scope.write(':CHANnel1:DISPlay ON')
    scope.write(':CHANnel1:COUPling AC')
    scope.write(':CHANnel1:SCALe 0.010')
    scope.write(':CHANnel1:BWLimit ON')
    scope.write(':CHANnel1:PROBe 10')
    
    scope.write(':CHANnel2:DISPlay ON')
    scope.write(':CHANnel2:COUPling DC')
    scope.write(':CHANnel2:SCALe 1.0')
    scope.write(':CHANnel2:OFFSet 1.5')
    scope.write(':CHANnel2:PROBe 10')
    
    scope.write(':TRIGger:MODE EDGE')
    scope.write(':TRIGger:EDGE:SOURce CHAN2')
    scope.write(':TRIGger:EDGE:SLOPe POSitive')
    scope.write(':TRIGger:EDGE:LEVel 1.5')
    scope.write(':TRIGger:SWEep NORMal')
    
    scope.write(':ACQuire:TYPE HRESolution')
    
    scope.write(':WAVeform:SOURce CHAN1')
    scope.write(':WAVeform:FORMat BYTE')
    scope.write(':WAVeform:POINts:MODE RAW')
    scope.write(f':WAVeform:POINts {REQUESTED_POINTS}')
    time.sleep(0.5)

def capture_trace(scope):
    scope.write(':SINGle')
    scope.query('*OPC?')
    
    preamble = scope.query(':WAVeform:PREamble?').split(',')
    y_inc = float(preamble[7])
    y_orig = float(preamble[8])
    y_ref = float(preamble[9])
    
    raw = scope.query_binary_values(':WAVeform:DATA?', datatype='B', container=np.array)
    volts = (raw - y_ref) * y_inc + y_orig
    return volts.astype(np.float32)

def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    print(f"Generating {N_TRACES} plaintexts using LFSR...")
    plaintexts = generate_plaintexts(N_TRACES)
    print(f"First plaintext:  {plaintexts[0].tolist()}")
    print(f"Second plaintext: {plaintexts[1].tolist()}")
    
    rm = pyvisa.ResourceManager()
    scope = rm.open_resource(SCOPE_ADDR)
    scope.timeout = 10000
    scope.clear()
    
    print("Connected:", scope.query('*IDN?').strip())
    configure_scope(scope)
    
    # ===== Capture first trace to determine actual length =====
    print("\nCapturing first trace to determine size...")
    try:
        first_trace = capture_trace(scope)
    except Exception as e:
        print(f"First capture failed: {e}")
        scope.close()
        return
    
    actual_points = len(first_trace)
    print(f"Scope returns {actual_points} points per trace")
    
    # Pre-allocate based on actual size
    traces = np.zeros((N_TRACES, actual_points), dtype=np.float32)
    traces[0] = first_trace
    
    print(f"\nCapturing {N_TRACES} traces...")
    t0 = time.time()
    i = 0
    
    try:
        for i in range(1, N_TRACES):
            try:
                trace = capture_trace(scope)
                
                # Handle occasional length mismatch by truncating/padding
                if len(trace) == actual_points:
                    traces[i] = trace
                elif len(trace) > actual_points:
                    traces[i] = trace[:actual_points]  # truncate
                else:
                    traces[i, :len(trace)] = trace     # pad with zeros
                    print(f"  Warning: trace {i} had {len(trace)} points (expected {actual_points})")
                    
            except Exception as e:
                print(f"\nError at trace {i}: {e}")
                print("Retrying...")
                time.sleep(1)
                scope.clear()
                try:
                    trace = capture_trace(scope)
                    if len(trace) == actual_points:
                        traces[i] = trace
                    else:
                        traces[i, :min(len(trace), actual_points)] = trace[:actual_points]
                except Exception as e2:
                    print(f"Retry failed: {e2}")
                    print(f"Stopping at trace {i}")
                    traces = traces[:i]
                    plaintexts = plaintexts[:i]
                    break
            
            if (i + 1) % 50 == 0 or (i + 1) == N_TRACES:
                elapsed = time.time() - t0
                rate = (i + 1) / elapsed
                eta = (N_TRACES - i - 1) / rate if rate > 0 else 0
                print(f"  Trace {i+1}/{N_TRACES}  ({rate:.1f} traces/sec, ETA: {eta:.0f}s)")
    
    except KeyboardInterrupt:
        print(f"\nInterrupted at trace {i}, saving what we have...")
        traces = traces[:i]
        plaintexts = plaintexts[:i]
    
    elapsed = time.time() - t0
    print(f"\nCaptured {len(traces)} traces in {elapsed:.1f} seconds")
    
    if len(traces) > 0:
        print("Saving traces...")
        np.save(os.path.join(OUTPUT_DIR, 'traces.npy'), traces)
        np.save(os.path.join(OUTPUT_DIR, 'plaintexts.npy'), plaintexts[:len(traces)])
        print(f"Saved to {OUTPUT_DIR}/")
        print(f"  traces.npy: shape {traces.shape}, size {traces.nbytes / 1e6:.1f} MB")
        print(f"  plaintexts.npy: shape {plaintexts[:len(traces)].shape}")
    else:
        print("No traces captured — nothing to save.")
    
    scope.write(':RUN')
    scope.close()

if __name__ == '__main__':
    main()